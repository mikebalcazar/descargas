...
curl -s -o /dev/null -w "%{http_code}\n" -H "Authorization: Bearer $T" \
  https://api.github.com/repos/mikebalcazar/descargas
```
...
```bash
git clone "https://x-access-token:${T}@github.com/mikebalcazar/descargas.git" /home/claude/repo
...
curl -s -H "Authorization: Bearer $T" \
  "https://api.github.com/repos/mikebalcazar/descargas/actions/runs?per_page=3" \
...
curl -s -X POST -H "Authorization: Bearer $T" \
  https://api.github.com/repos/mikebalcazar/descargas/pulls \
  -d '{"title":"…","head":"claude/…","base":"main","body":"…"}'
# y con el número que devuelve:
curl -s -X PUT -H "Authorization: Bearer $T" \
  https://api.github.com/repos/mikebalcazar/descargas/pulls/<N>/merge \
...
curl -s -H "Authorization: Bearer $T" \
  "https://api.github.com/repos/mikebalcazar/descargas/commits/<SHA>/comments" \
  | python3 -c "import json,sys; [print(c['body']) for c in json.load(sys.stdin)]"

# y si algo falló, en qué trabajo fue
curl -s -H "Authorization: Bearer $T" \
  "https://api.github.com/repos/mikebalcazar/descargas/actions/runs/<ID>/jobs" \
...
```bash
curl -s -H "Authorization: Bearer $T" \
  "https://api.github.com/repos/mikebalcazar/descargas/actions/permissions/workflow"
```
...
```bash
curl -s -X POST -H "Authorization: Bearer $T" \
  "https://api.github.com/repos/mikebalcazar/descargas/actions/workflows/verificar.yml/dispatches" \
